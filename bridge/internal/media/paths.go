// Package media confines outbound media paths to an allow-list.
//
// /api/send accepts an absolute path in media_path and the bridge reads it
// via os.ReadFile. Unconstrained, that makes the bridge a generic
...
// ResolveRoots returns the list of allowed absolute root directories
...
// ensure existence first (see ResolveRoots).
func canonicalizePath(p string) (string, error) {
	...
	if err != nil {
		...
		// ValidatePath will run EvalSymlinks again at request time
...
// ValidatePath checks whether mediaPath points at a regular file inside one
// of the allowed roots. Returns the canonical path on success so callers
// read from the resolved file (defends against TOCTOU between validation
// and ReadFile in degenerate cases).
func ValidatePath(mediaPath string, allowedRoots []string) (string, error) {
	if mediaPath == "" {
