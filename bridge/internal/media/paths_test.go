...
func TestValidatePathAcceptsFileInsideRoot(t *testing.T) {
	root := t.TempDir()
	...
	got, err := ValidatePath(f, []string{resolvedRoot})
	if err != nil {
...
func TestValidatePathRejectsOutsideRoot(t *testing.T) {
	...
	if _, err := ValidatePath(f, []string{resolvedRoot}); err == nil {
...
func TestValidatePathRejectsSymlinkEscape(t *testing.T) {
	...
	if _, err := ValidatePath(link, []string{resolvedRoot}); err == nil {
...
func TestValidatePathRejectsRelative(t *testing.T) {
	if _, err := ValidatePath("not/absolute.txt", []string{"/tmp"}); err == nil {
...
func TestValidatePathRejectsDirectory(t *testing.T) {
	...
	if _, err := ValidatePath(subdir, []string{resolvedRoot}); err == nil {
		t.Fatal("expected error for directory, got nil")
	}
}

func TestResolveRootsRejectsRelativeEnv(t *testing.T) {
...
func TestResolveRootsAcceptsEnvList(t *testing.T) {
	a := t.TempDir()
	b := t.TempDir()
