"""Tests for the bridge REST client: token discovery and authenticated POSTs."""

import pytest

from whatsapp_mcp.bridge import client
from whatsapp_mcp.core import config
...
def test_bridge_headers_uses_env_token(monkeypatch):
    ...
    assert client._bridge_headers() == {"Authorization": "Bearer env-token"}
...
def test_bridge_headers_falls_back_to_token_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(client, "_BRIDGE_TOKEN_PATH", str(token_file))

    assert client._bridge_headers() == {"Authorization": "Bearer file-token"}
...
def test_bridge_headers_reads_token_next_to_whatsmeow_db_path(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("WHATSAPP_BRIDGE_TOKEN", raising=False)
    monkeypatch.setattr(config, "WHATSMEOW_DB_PATH", str(store_dir / "whatsapp.db"))

    assert client._bridge_headers() == {"Authorization": "Bearer volume-token"}
...
def test_send_message_without_token_surfaces_bridge_401(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("WHATSAPP_BRIDGE_TOKEN", raising=False)
    monkeypatch.setattr(client, "_BRIDGE_TOKEN_PATH", str(missing_token))
    ...
    success, message = client.send_message("12025551234", "hello")
...
def test_bridge_post_helpers_include_auth_headers(monkeypatch, tmp_path, func_name, args, expected_suffix):
    ...
    getattr(client, func_name)(*resolved_args)
...
def test_send_reaction_posts_correct_payload(monkeypatch):
    ...
    success, msg = client.send_reaction(
...
def test_send_reaction_empty_emoji_sends_removal(monkeypatch):
    ...
    success, _ = client.send_reaction("12025551234@s.whatsapp.net", "3AABCDEF01234567", "")
...
def test_send_reaction_missing_recipient_returns_error():
    ...
    success, msg = client.send_reaction("", "3AABCDEF01234567", "👍")
    assert success is False
...
def test_send_reaction_missing_message_id_returns_error():
    ...
    success, msg = client.send_reaction("12025551234@s.whatsapp.net", "", "👍")
    assert success is False
...
def test_send_message_without_quote_omits_quote_fields(monkeypatch):
    ...
    client.send_message("12025551234@s.whatsapp.net", "Hello!")
